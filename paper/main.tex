...
The increasing use of facial images in computational studies has created a methodological tension. On the one hand, researchers often need to compare images, evaluate group-level patterns, or audit visual representations produced by generative systems. On the other hand, many of these analyses involve socially sensitive categories, such as gender, ethnicity, or other appearance-related group labels, where direct human labeling can introduce subjective, culturally situated, and potentially harmful judgments \cite{buolamwini2018gender,mehrabi2021survey,heidari2016sager}. Fully automated deep models may reduce manual annotation costs, but they frequently operate through high-dimensional representations that are difficult to inspect, making it unclear which visual cues support a decision \cite{schroff2015facenet,clip}.

The state of the art in facial image analysis has been strongly shaped by deep representation learning. Systems such as DeepFace and FaceNet demonstrated that neural networks can encode facial images into compact feature spaces that are highly effective for verification, clustering, and similarity estimation \cite{taigman2014deepface,schroff2015facenet}. More recent multimodal models extended this paradigm by learning image representations from large-scale image-text supervision, enabling broad semantic comparison across visual domains \cite{clip}. These approaches are powerful, but their internal coordinates are not designed to correspond to named facial attributes. As a result, they often provide accurate similarity scores without a direct account of which observable facial properties contributed to the comparison.

In parallel, the literature on AI fairness and representation has shown that visual systems can encode uneven demographic performance, stereotyped associations, and culturally narrow depictions of people \cite{buolamwini2018gender,mehrabi2021survey,luccioni2023stablebias}. This is particularly relevant for generative AI, where synthetic portraits may be used as evidence of social representation despite being produced by systems trained on large and uneven visual corpora. Interpretability research has therefore argued for methods whose decisions can be inspected, contested, and evaluated in relation to meaningful features rather than only post hoc explanations of black-box models \cite{doshi2017rigorous,rudin2019stop}. For studies involving sensitive labels, this creates a need for methods that can support group-level auditing while making their assumptions and feature contributions explicit.
