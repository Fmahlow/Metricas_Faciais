...
\subsection{Facial Representation and Recognition}

Facial image analysis has been largely shaped by deep representation learning. Early systems such as DeepFace and FaceNet established the effectiveness of mapping facial images into compact latent spaces for verification, clustering, and similarity estimation. Subsequent methods refined this paradigm through angular and margin-based objectives, including SphereFace, CosFace, and ArcFace, which improved intra-class compactness and inter-class separation in face embeddings. More recent approaches have further addressed training dynamics, image quality, and sample difficulty, as seen in CurricularFace, MagFace, AdaFace, ElasticFace, and related quality-aware or adaptive-margin methods. Large-scale benchmarks such as WebFace260M have also reinforced the central role of data scale in modern face recognition. In parallel, multimodal models such as CLIP have extended visual representation learning beyond identity-centered face recognition toward broad image-text semantic alignment. Despite their strong performance, these methods generally optimize latent representations for discrimination or retrieval rather than for correspondence with named, inspectable facial attributes.

\subsection{Bias, Fairness, and Visual Representation Auditing}

A growing body of work has shown that facial and visual AI systems can encode demographic disparities, uneven performance, and representational harms. Gender Shades demonstrated intersectional accuracy disparities in commercial gender classification systems, while large-scale evaluations such as NIST FRVT documented demographic effects across face recognition algorithms. Datasets and benchmarks such as FairFace and RFW were introduced to measure or mitigate demographic imbalance in facial analysis. More recently, the focus has expanded from recognition systems to generative visual models. Studies such as Stable Bias, Social Biases through the Text-to-Image Generation Lens, T2IAT, OpenBias, and large-scale gender-bias analyses of text-to-image systems show that generated portraits can reproduce occupational stereotypes, gender imbalance, racial homogenization, and culturally narrow visual tropes. These works motivate audit methods that are explicit about categories, labels, and measurement assumptions. However, many existing audits rely on external classifiers, latent embeddings, or semantic labels, making it difficult to inspect which concrete visual features contributed to a given representational pattern.

\subsection{Interpretable and Geometry-Based Facial Analysis}

Before the dominance of deep embeddings, facial analysis often relied on explicit geometric models, including active appearance models, 3D morphable models, landmark localization, and face alignment. These methods made facial structure more inspectable by representing faces through landmarks, shapes, meshes, or parametric geometry. Modern approaches such as ensemble-regression landmark alignment, Face Alignment Networks, MediaPipe Face Mesh, 3DDFA-V2, DECA, and EMOCA continue this geometric tradition while improving robustness in unconstrained images, 3D reconstruction, and expression-aware modeling. Recent segmentation foundation models, especially Segment Anything, provide another path toward explicit region-based representations by extracting object or part masks that can be converted into measurable geometric quantities. SI-FPM builds on this interpretable lineage but differs from landmark-only and embedding-based approaches by representing segmented facial regions through pairwise proportional ratios and by providing feature-level drivers for group-proximity decisions. This makes the metric suitable for audit settings where both measurement transparency and ethical caution are required.
