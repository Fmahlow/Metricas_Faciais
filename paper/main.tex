...
Modern facial image analysis has been strongly shaped by deep representation learning. DeepFace showed that a deep neural network trained on large face datasets could approach human-level performance in face verification, while FaceNet formalized a widely influential embedding paradigm in which facial images are mapped to compact vectors optimized for verification, recognition, and clustering \cite{taigman2014deepface,schroff2015facenet}. These systems changed the methodological center of facial analysis: rather than working with explicitly named measurements, they learned high-dimensional representations whose coordinates are optimized for discrimination. This made facial comparison scalable and highly effective, but it also made the resulting similarity scores difficult to interpret in terms of observable facial properties.

Subsequent work improved face recognition by modifying the geometry of the embedding space. SphereFace introduced angular margins on a hypersphere, CosFace used a cosine margin to increase class separation, and ArcFace proposed an additive angular margin with a direct geometric interpretation on the hypersphere \cite{liu2017sphereface,wang2018cosface,deng2019arcface}. Later methods refined this line of work by considering sample difficulty, image quality, and margin flexibility. CurricularFace adjusted the learning emphasis from easy to hard samples over training, MagFace connected feature magnitude to face quality, AdaFace adapted the margin according to estimated image quality, and ElasticFace randomized the margin to improve generalization \cite{huang2020curricularface,meng2021magface,kim2022adaface,boutros2022elasticface}. Large-scale benchmarks such as WebFace260M further showed how data scale and cleaning protocols influence recognition performance \cite{zhu2021webface260m}.

These advances define much of the current technical landscape, but they address a different problem from the one considered here. Their main objective is to produce discriminative embeddings for recognition or retrieval, not to expose which facial proportions support a comparison. Multimodal representation learning, exemplified by CLIP, extends this embedding logic to broad image-text alignment and semantic retrieval, but its latent dimensions are also not designed to correspond to named facial measurements \cite{clip}. SI-FPM therefore does not attempt to replace high-performance recognition systems. It targets audit settings in which a lower-dimensional, explicitly named, and inspectable representation is preferable to a more powerful but opaque embedding.

\subsection{Bias, Fairness, and Visual Representation Auditing}

The fairness literature has shown that facial analysis systems can produce uneven performance across demographic groups and that these disparities are often intersectional. Gender Shades demonstrated large error differences in commercial gender classification systems, especially for darker-skinned women, and NIST's Face Recognition Vendor Test reported demographic effects across face recognition algorithms at scale \cite{buolamwini2018gender,grother2019frvt}. Dataset-oriented responses such as Racial Faces in the Wild and FairFace were proposed to evaluate or mitigate imbalance across race, gender, and age annotations \cite{wang2019rfw,karkkainen2021fairface}. Broader surveys of bias and fairness in machine learning emphasize that these effects are not merely technical artifacts; they reflect choices about datasets, labels, objectives, evaluation protocols, and deployment contexts \cite{mehrabi2021survey}.

Recent work has extended these concerns from recognition systems to generative visual models. Stable Bias examined social representations in text-to-image systems by comparing generated images across gender, ethnicity, and professions, while Naik and Nushi studied occupational, personality, and geographic biases in generated images from DALL-E 2 and Stable Diffusion \cite{luccioni2023stablebias,naik2023socialbiases}. T2IAT adapted the logic of implicit association testing to text-to-image generation, measuring valence and stereotypical associations in generated outputs \cite{wang2023t2iat}. More recent auditing frameworks have broadened the scope of analysis: OpenBias detects open-set biases without requiring a fixed bias list, large-scale gender-bias studies evaluate millions of generated images across everyday prompts, and country-aware analyses compare synthetic demographic representations with geographically grounded baselines \cite{dinca2024openbias,girrbach2025largescale,franco2026biasbeyondborders}.

This literature motivates SI-FPM in two ways. First, it shows that visual systems should be audited at the level of representation, not only at the level of task accuracy. Second, it shows that sensitive labels require careful operational framing, because synthetic portraits and dataset annotations do not possess intrinsic gender, ethnicity, or race. Many existing audits rely on external classifiers, vision-language models, or latent embeddings to assign categories or measure similarity. Those tools can be useful, but they may leave unclear which concrete visual properties drive the audit result. SI-FPM addresses this gap by making each feature a named facial proportion and by reporting feature-level drivers for group-proximity decisions.

\subsection{Interpretable and Geometry-Based Facial Analysis}

A separate line of work has represented faces through explicit geometry rather than opaque embeddings. Active Appearance Models combined shape and texture variation in a statistical model, while 3D Morphable Models represented facial shape and texture through parametric 3D face spaces \cite{cootes2001aam,blanz1999morphable}. Landmark-based methods then made facial geometry operational in 2D and 3D image analysis. Kazemi and Sullivan proposed a fast ensemble-of-regression-trees method for face alignment, and Bulat and Tzimiropoulos showed that deep face alignment networks could approach strong performance across 2D and 3D landmark datasets \cite{kazemi2014facealignment,bulat2017facealignment}. These methods are more inspectable than generic embeddings because their outputs correspond to points, shapes, or model parameters, but they still depend on the stability and semantic adequacy of the detected landmarks.

More recent geometric systems have improved the density, robustness, and realism of facial structure estimation. MediaPipe Face Mesh introduced real-time facial surface geometry from monocular video, and Attention Mesh improved high-fidelity prediction for semantically important regions such as eyes and lips \cite{kartynnik2019facemesh,grishchenko2020attentionmesh}. 3DDFA-V2 targeted fast, accurate, and stable dense 3D face alignment, while DECA and EMOCA advanced monocular 3D face capture by modeling detailed expression and emotion-related geometry \cite{guo2020threeddfa,feng2021deca,danecek2022emoca}. These methods demonstrate that explicit facial geometry remains important even in the deep learning era, especially when the goal is to preserve structural information rather than only produce a recognition score.

SI-FPM builds on this interpretable tradition but uses segmentation rather than landmarks or dense meshes as its primary input. Foundation segmentation models such as the Segment Anything Model can produce region-level masks that are directly convertible into area-based measurements \cite{kirillov2023segment}. Once facial regions are segmented, pairwise ratios provide a scale-normalized representation in which each coordinate has a semantic label. This design is aligned with interpretability arguments that, in high-stakes or socially sensitive settings, explanations should be tied to meaningful features rather than only post hoc descriptions of black-box behavior \cite{doshi2017rigorous,rudin2019stop}. The resulting metric is therefore not only a facial descriptor, but an audit mechanism whose assumptions, variables, and local drivers can be inspected.
